...
class PiT(BaseTransformerModel):
    ...
    def __init__(self, arch_config = None, num_classes = None):
        ...
        self.hidden_dim = [self.base_dim * self.num_heads[i] for i in range(len(self.depth))]
        ...
        self.pos_embed = nn.Parameter(
            torch.randn(1, self.base_dim * self.num_heads[0], embed_size, embed_size),
            requires_grad=True
        )
        self.patch_embed = conv_embedding(self.in_channels, self.base_dim * self.num_heads[0],
                                          self.patch_size, self.stride, 0)

        self.num_tokens = 1 + cfg.DISTILLATION.ENABLE_LOGIT

        self.cls_token = nn.Parameter(torch.randn(1, self.num_tokens, self.base_dim * self.num_heads[0]))
        ...
        for stage in range(len(self.depth)):
            ...
            self.transformers.append(
                Transformer(self.base_dim * self.num_heads[stage], self.depth[stage], self.num_heads[stage],
                            self.mlp_ratio,
                            self.drop_rate, self.attn_drop_rate, drop_path_prob)
            )
            if stage < len(self.depth) - 1:
                self.pools.append(
                    conv_head_pooling(self.base_dim * self.num_heads[stage], self.base_dim * self.num_heads[stage + 1], stride=2))
        ...
        self.head = nn.Linear(self.base_dim * self.num_heads[-1], self.num_classes)

        if cfg.DISTILLATION.ENABLE_LOGIT:
            self.distill_head = nn.Linear(self.embed_dim, self.num_classes)
    ...
    def forward(self, x):
        ...
        self.distill_logits = self.distill_head(cls_token[:, 1])
        if self.training:
            return x_cls
        else:
            return (x_cls + self.distill_logits) / 2
