...
def sample_configs(choices):
    ...
    config['hidden_dim'] = [random.choice(choices['hidden_dim'])]*depth
...
def train_epoch(loader, model, ema, loss_fun, optimizer, scheduler, scaler, meter, cur_epoch):
    ...
    for cur_iter, (inputs, labels, offline_features) in enumerate(loader):
        ...
        if cfg.AUTOFORMER_MODE == 'super':
            choices = {'num_heads': cfg.AUTOFORMER_SEARCH_SPACE.NUM_HEADS, 'mlp_ratio': cfg.AUTOFORMER_SEARCH_SPACE.MLP_RATIO,
                       'hidden_dim': cfg.AUTOFORMER_SEARCH_SPACE.HIDDEN_DIM, 'depth': cfg.AUTOFORMER_SEARCH_SPACE.DEPTH}
            config = sample_configs(choices = choices)
            model_module = net.unwrap_model(model)
            model_module.set_sample_config(config=config)

        elif cfg.AUTOFORMER_MODE == 'retrain':
            retrain_config = {'depth': cfg.AUTOFORMER_SUBNET.DEPTH, 'hidden_dim': [cfg.AUTOFORMER_SUBNET.HIDDEN_DIM] * cfg.AUTOFORMER_SUBNET.DEPTH,
                              'num_heads': cfg.AUTOFORMER_SUBNET.NUM_HEADS, 'mlp_ratio': cfg.AUTOFORMER_SUBNET.MLP_RATIO}
            model_module = net.unwrap_model(model)
            model_module.set_sample_config(config=retrain_config)
...
@torch.no_grad()
def test_epoch(loader, model, meter, cur_epoch,  sub_arch = None):
    model.eval()
    if not sub_arch and cfg.AUTOFORMER_MODE == 'super':
        choices = {'num_heads': cfg.AUTOFORMER_SEARCH_SPACE.NUM_HEADS,
                   'mlp_ratio': cfg.AUTOFORMER_SEARCH_SPACE.MLP_RATIO,
                   'hidden_dim': cfg.AUTOFORMER_SEARCH_SPACE.HIDDEN_DIM, 'depth': cfg.AUTOFORMER_SEARCH_SPACE.DEPTH}
        config = sample_configs(choices=choices)
        model_module = unwrap_model(model)
        model_module.set_sample_config(config=config)

    elif not sub_arch and  cfg.AUTOFORMER_MODE == 'retrain':
        config = {'depth': cfg.AUTOFORMER_SUBNET.DEPTH,
                  'hidden_dim': [cfg.AUTOFORMER_SUBNET.HIDDEN_DIM] * cfg.AUTOFORMER_SUBNET.DEPTH,
                  'num_heads': cfg.AUTOFORMER_SUBNET.NUM_HEADS, 'mlp_ratio': cfg.AUTOFORMER_SUBNET.MLP_RATIO}
        model_module = unwrap_model(model)
        model_module.set_sample_config(config=config)
    ...
    logger.info("sampled model config: {}".format(config))
    parameters = model_module.get_sampled_params_numel(config)
    logger.info("sampled model parameters: {}".format(f'{round(parameters/1e6, 2)}M'))
...
def test_model(sub_arch=None):
    ...
    test_loader = data_loader.construct_test_loader()
    test_meter = meters.TestMeter(len(test_loader))
    test_epoch(test_loader, model, test_meter, 0, sub_arch)
    test_err = test_meter.get_epoch_stats(0)["top1_err"]
    logger.info("Test top-1 err: {}".format(test_err))
    # with open(os.path.join(cfg.OUT_DIR, 'test.txt'), 'w') as f:
    #     json.dump(test_err,f)
